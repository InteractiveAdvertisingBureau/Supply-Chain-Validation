...
|is_passthrough |	integer; optional, default 0 |	A passthrough seller is a facilitator of inventory from the upstream supplier to the consumer of the inventory. Where the upstream supplier and consumer have an account control relationship, such that the upstream supplier has control of their account within the consumer’s platform, this field signals that fact. <br><br> **A value of 1** indicates the following: <br>- This seller has an account control relationship with the downstream/consuming advertising system. <br>-	If this seller is the last link in a SupplyChain, the buying system has an account control relationship with this seller. <br>-	If this is not the last link in a SupplyChain than this seller should exist between two entities that have an account control relationship. <br><br> This should be set to 1 for the seller to be used as a non-payment node (hp=0) in Object: Supply Chain.<br><br> **A value of 0** indicates: <br>- The downstream/consuming advertising system has no account control relationship with this seller.  |
